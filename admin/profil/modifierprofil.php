...
if (isset($_POST['nom']) and isset($_POST['action']) and isset($_POST['mdp']) and isset($_POST['confirm'])){
  if (!empty($_POST['nom']) and !empty($_POST['action']) and !empty($_POST['mdp']) and  !empty($_POST['confirm']) and ($_POST['mdp']==$_POST['confirm']) and (strlen($_POST['mdp'])>=8)){
  $con=mysqli_connect('localhost','root','','autohall') or die('Impossible d\'accerder auserveur');
  $sql="UPDATE admin
  SET nom ='".$_POST['nom']."', action ='".$_POST['action']."', mdp ='".$_POST['mdp']."'
  where email ='".$_GET['id']."' ";
...
$sql="select * from admin where email='".$_GET['id']."'";
$r=mysqli_query($con,$sql) or die('erreur exec recet');
$d=mysqli_fetch_array($r);
?>
<section class="table">
<div class="tableau">
  <h1>Profil de <?php echo $d['nom']; ?></h1>
  <br>
